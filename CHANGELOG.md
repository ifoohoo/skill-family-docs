...
- 收紧 setup 既有接入事务：先展示完整、可复核的安装计划，再请求用户授权。计划绑定精确版本、来源、命令、环境变量、配置值、写入路径与成功信号；授权前不写项目、缓存、日志或宿主持久状态。
- 执行前重新核对整份计划。配置值尚未确定或项目事实已变化时，setup 不执行任何计划前缀，而是重新出具计划。
- 已有自定义站点按保留现状、兼容接入、冲突和显式迁移分流。配置完整但正文尚未生成时，状态明确报告为“接入配置完成，等待首次生成”。

### 变更

- 四类插件清单与平台清单统一更新为 0.4.5。

## 0.4.4 - 2026-09-10

### 改进

- setup 改为连续的“检查、精确计划、影响说明、授权、执行、复验”事务。裸调用发现缺项时会说明官方版本来源、完整安装命令和文件影响，再询问是否执行；授权前不写项目或旁路状态，Git 查询也禁用 optional locks，避免刷新索引。已有自定义站点按保留现状、兼容接入、有冲突和显式迁移分流。

...
description: skill-family-docs 插件技能：只读检查公开文档站渲染环境是否就绪。检查安装闭包（skill-family-doc-render npm 包）、Node 与 CLI 可用性、public-release.json、站点源目录和渲染前置条件；环境就绪时明确报告无需变更。只读检查：不自动安装依赖、不创建或修改配置、不执行渲染写盘、不调用发布动作。仅在公开文档站领域使用：首次接入、环境异常诊断或渲染前就绪确认时。
---
...
2. **CLI 可用性**：按顺序找一个已存在的 `skill-family-doc-render` 命令，只执行找到的那一个：
   ...
   用该命令执行 `--help`，应输出用法并退出 0；`--help` 是只读命令。两条路都找不到时，只报告 CLI 缺失与安装 `skill-family-doc-render@0.2.0` 的方向，不执行安装、不访问 registry。第 5 步复用本步发现的同一命令。
3. **工作区配置**：渲染工作区根存在 `public-release.json` 且是可解析 JSON。只读取校验，不创建、不修改。
4. **站点源完整性**：对 `public-release.json` 里每个带 `site` 字段的 repo 确认：
...
5. **基线状态**：用第 2 步发现的同一命令执行 `--check`（只读校验，不写盘）。报漂移说明源与产物不一致、需要重渲染——重渲染属 `skill-family-docs-render-site`，不在本技能范围。
